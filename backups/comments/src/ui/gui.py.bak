...
class OptimizedStyleManager:
    ...
    def colors(self) -> Dict[str, str]:
        if self._colors is None:
            # Test to take system -wide settings into account (e.g. Dark/Light Mode)
    ...
    def fonts(self) -> Dict[str, tuple]:
        if self._fonts is None:
            # System-dependent font with fallback options
            if platform.system() == 'Windows':
            ...
            # System -dependent font size adjustment
            size_adjust = 0
            if platform.system() == 'Darwin':  # MacOS uses larger fonts
                size_adjust = 1
...
class MemoryEfficientProgressBar(ttk.Progressbar):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self._update_queue = []
        self._parent_ref = weakref.ref(parent)  # Weak reference to the parent element
        self._adaptive_mode = kwargs.get('adaptive_mode', True)
        self._frame_times = deque(maxlen=5)

        # Performance monitoring for adaptive adjustments
    ...
    def update_throttled(self, value):
        ...
        if self._adaptive_mode and current_time - self._performance_metrics['last_ui_check'] > 2.0:
            ...
            if parent:
                # Carry out the UI thread load test
                self._check_ui_responsiveness(parent)

        # Use the adaptive settings for update threshold and batch size
        ...
        if current_time - self._last_update >= update_threshold:
            # Significant change or long without update -> update immediately
            if abs(value - (self['value'] or 0)) > 5 or current_time - self._last_update >= 0.5:
                ...
                # Put update in the queue to reduce Ui-O-O-ORHEAD
                self._update_queue.append(value)
                self._performance_metrics['updates_batched'] += 1

                # Process the batch if there is enough entries or if too much time has passed since the last update
                if (len(self._update_queue) >= batch_size or
                    current_time - self._last_update >= update_threshold * 3):
                    try:
                        # Used average with more weight in recent values
                        ...
                        # Widget may be destroyed or empty queue
                        self._update_queue.clear()
                        return False
        else:
            # Update is skipped
    ...
    def _check_ui_responsiveness(self, parent):
        """Checks the UI responsiveness and adjusts settings."""
        try:
            # Calculate current refresh rate if there are enough data points
            if len(self._frame_times) >= 2:
                ...
                # Adaptation of the settings according to power metrics
                now = time.time()
                if now - cls._adaptive_settings['last_performance_check'] > 5.0:
                    ...
                    if cls._adaptive_settings['frame_rate_history']:
                        ...
                        # UI overload recognition and adaptation
                        if avg_fps < 15:  # The UI is considered overloaded under 15 fps
                            cls._adaptive_settings['ui_congestion_detected'] = True
                            # Recover update threshold and batch size for fewer UI updates
                        ...
                        elif avg_fps > 30:  # Ui is liquid
                            cls._adaptive_settings['ui_congestion_detected'] = False
                            # Reduce update threshold and batch size for more responsive UI
...
class SmartThread(threading.Thread):
    ...
    # Class variables for monitoring active threads
    ...
    # Thread pool for reusable worker
    _thread_pool = None
    ...
    def __init__(self, target=None, args=(), kwargs=None, **thread_kwargs):
        ...
        # Use a proxy for the target function for monitoring
        if target:
    ...
    def _wrap_target(self, target):
        """Wraps the target function with error and resource monitoring."""
        def wrapped_target(*args, **kwargs):
            try:
                ...
                # Record the mistake, but do not throw it up
    ...
    def get_execution_time(self):
        """Returns the execution time of the thread."""
        ...
        with cls._pool_lock:
            if cls._thread_pool is None:
                if max_workers is None:
                    # Optimal pool size based on system
    ...
    def _cleanup(self):
        ...
        # Delete references to functions and arguments
...
class OptimizedDragDropFrame(tk.Frame):
    ...
    def _create_content(self):
        ...
        if DND_AVAILABLE:
            sub_text = f"Drag & Drop supported ({DND_MODE})"
            ...
            try:
                ...
                if DND_MODE == "native":
                    ...
                    if not hasattr(self.winfo_toplevel(), 'TkdndVersion'):
                        print("Root window does not support tkinterdnd, trying to patch")

                        # Try to patch the root class
                        try:
                            ...
                            print("Root window successfully patched")
                        except Exception as patch_err:
                            print(f"Failed to patch root window: {patch_err}")

                    # Registriere als Drag & Drop-Ziel
                    self.drop_target_register(DND_FILES)
                    self.dnd_bind('<<Drop>>', self._on_drop)

                    # Register additional events for visual feedback
                    ...
                    print("TkinterDnD Drag & Drop support successfully registered")

                print("Drag & Drop support active")
            except Exception as e:
                print(f"Drag & Drop-Setup fehlgeschlagen: {e}")
                import traceback
                traceback.print_exc()
                # Update variable in the outer scope
    ...
    def _on_click(self, event=None):
        ...
        if self.callback:
            folder = filedialog.askdirectory(
                title="Select folder with ROM files",
                mustexist=True
            )
            if folder:
    ...
    def _drag_enter(self, event=None):
        ...
        # Lively visual feedback for drag & drop
        self.configure(
    ...
    def _handle_native_drop(self, file_paths):
        ...
        try:
            ...
            # Priority 1: Search for directories
            for path in file_paths:
                try:
                    if os.path.exists(path) and os.path.isdir(path):
                        print(f"Valid directory found: {path}")
                        self.callback(path)
            ...
            # Priority 2: Use parent lists of files
            for path in file_paths:
                try:
                    ...
                    print(f"Error in parent directory processing: {parent_err}")

            # If no valid directories were found
            print("No valid directory found in drop data")
            self._show_feedback("No valid folder", False)
    ...
    def _on_drop(self, event):
        ...
        try:
            ...
            # Debug edition for all event details
            ...
            if not paths:
                print("No valid paths extrahiert")
                self._show_feedback("No valid paths", False)
                return
            ...
            # Priority 3: Special URL format treatment
            for path in paths:
                try:
                    # Remove URL prefix if available
    ...
    def _extract_paths_from_drop(self, data):
        """Enhanced and more robust path extraction for different operating systems and Drag&Drop formats."""
        if not data:
            return []
        ...
        if isinstance(data, (list, tuple)):
            ...
            if os.name == 'nt' and data_str.startswith('{') and data_str.endswith('}') and ':\\' in data_str:
                # Windows Explorer Format with a single path in curly brackets
                clean_path = data_str.strip('{}').strip()
                return [clean_path]

            # Remove Windows-specific cute clips (for several entries)
            if data_str.startswith('{') and data_str.endswith('}'):
                data_str = data_str[1:-1].strip()

            # Try to recognize different separator
            if '\r\n' in data_str:
                ...
                # Windows with quotation marks
                try:
                    # Attempts Shlex for correct quotation sign treatment
                    import shlex
                    data_items = shlex.split(data_str)
                except Exception:
                    # Fallback in the event of errors
                    data_items = []
                    # Treatment for quotation marks
                    in_quotes = False
                    quote_type = None
                    current_item = ""
                    ...
                    if current_item:  # Add the last entry
                        data_items.append(current_item)
            else:
        ...
        for item in data_items:
            ...
            if isinstance(item, str) and item.startswith('file:'):
                ...
                # In Windows, remove the leading slash
            ...
            if isinstance(path, str):
                # Remove quotation marks and unnecessary spaces
                clean_path = path.strip('"\'').strip()

                # Normalized backslashes/forwardslashes depending on the operating system
                if os.name == 'nt':
    ...
    def _reset_appearance(self):
        ...
        if DND_AVAILABLE:
            main_text = "📁 Drag folder here oder klicken"
            sub_text = "Drag & Drop supported"
        else:
            main_text = "📁 Click to select folder"
            sub_text = "Select ROM folder"
...
class OptimizedStatsWidget(tk.Frame):
    ...
    def update_stats(self, stats: Dict[str, int]):
        ...
        # Adjusted statistics (remove double keys that differ only by formatting)
    ...
    def _clean_console_stats(self, stats: Dict[str, int]) -> Dict[str, int]:
        """Cleans console statistics for consistent display and performance."""
        cleaned_stats = {}

        # Intermediate memory for name adjustment
        name_cache = {}

        # Compile regular expressions for frequent patterns
        ...
        # Ignore words that do not contribute to console identification
        ...
        # Console group for related variants
        if not hasattr(self, '_console_groups'):
            ...
            # Create reverse assignment for faster lookup
        ...
        # Normalize console names for better display
        for console, count in stats.items():
            # Replace underlined underlined by spaces for better readability
            display_name = console.replace('_', ' ')

            # Make sure that similar names are summarized
...
class FastFileScanner:
    ...
    def __init__(self, extensions=None):
        ...
        self._progress_interval = 50  # Initial interval for progress updates
        ...
        # Adaptive parameters for performance optimization
    ...
    def _optimized_extension_check(self, path):
        """Optimized check for ROM file extensions."""
        # Fastest approach for the expansion test
    ...
    def scan_directory(self, directory: str, progress_callback=None, stop_event=None, use_cache=False) -> List[Path]:
        """Scans directory for ROM files with optimal performance and adaptive adjustment."""
        self._file_count = 0
        self._dir_count = 0
        ...
        # Cache-Lookup with optimized key
                ...
                if cache_key in self._scan_cache:
                    ...
                    if time.time() - cache_time < cache_ttl:
                        ...
                        if valid and cached_files:
                            # Registration progress for Ui feedback also in the case of cache use
        ...
        # Directory exists before the scan
        ...
        if use_cache and found_files:
            ...
            with self._cache_lock:
                ...
                # Limit cache size to avoid memory problems
            ...
            # Update power metrics for future optimizations
            if total_time > 0 and self._file_count > 0:
                ...
                # Adapt adaptive parameters for future scans
    ...
    def _measure_filesystem_performance(self, directory):
        """Measures file system performance for adaptive optimization."""
        try:
            # Perform short I/O time measurement
            start = time.time()
            sample_size = 3
            count = 0
            ...
            if time.time() - self.__class__._performance_metrics['last_scan_time'] > 300:
                ...
                # If we have enough samples, calculate average latency
                ...
                if count > 0:
                    ...
                    # Adjust the batch size based on file system latency
            ...
            # Retain default values for errors
            pass

    def _should_use_threading(self, directory):
        """Determines whether threading should be used for this directory."""
        # Check whether threading is activated and sensible
        if not self._adaptive_params['threading_enabled'] or self._adaptive_params['is_slow_filesystem']:
            return False

        try:
            # Fast count of the top directory level
            dir_count = 0
            file_count = 0
            ...
            # 2. Sufficient potential files (based on sample)
    ...
    def _scan_sequential(self, directory, progress_callback=None):
        """Sequential scanning of the directory without threading."""
        found_files = []
        file_batch = []
        ...
        while dirs_queue and scan_depth < max_depth:
            ...
            try:
                ...
                # System directories and hidden directories skip
                ...
                try:
                    # More efficient directory scanning with scandir
                    with os.scandir(current_dir) as it:
                        ...
                        for entry in it:
                            ...
                            try:
                                # Efficient review on directory or file
                                if entry.is_dir():
                                    # Collect subdirectaries for later add
                                ...
                                # Skip files/directories that we cannot access
                                continue

                        # Add subdirectaries after the current directory has been scanned
                        dirs_queue.extend(subdirs)

                except (PermissionError, OSError):
                    # Skip lists that we cannot access
                    continue

            except IndexError:
                # Queue is empty, increase the deep level
                scan_depth += 1

        # Add remaining files in the batch
        if file_batch:
            found_files.extend(file_batch)
    ...
    def _scan_with_threading(self, directory, progress_callback=None):
        """Parallelized scanning of the directory with threading."""
        found_files = []
        all_dirs = []

        # First scan first directory level to distribute work
        try:
            with os.scandir(directory) as it:
                for entry in it:
                    if entry.is_dir():
                        ...
                        # Check files directly in the root
                        file_path = Path(entry.path)
                        if self._is_rom_file(file_path):
                            found_files.append(file_path)
                            self._file_count += 1
        except (PermissionError, OSError):
            # If there are problems with the root list to return to sequential scanning
            return self._scan_sequential(directory, progress_callback)

        if not all_dirs:
            # If no subdirectories are present, use sequential scanning
            return self._scan_sequential(directory, progress_callback)

        # Prepare thread pool
        max_threads = min(len(all_dirs), self.__class__._settings['max_threads'])
        if max_threads <= 1:
            # Not enough directories for effective threading
            return self._scan_sequential(directory, progress_callback)

        # Distribute directories to worker threads
        ...
        if self._thread_error:
            ...
            # Fallback for sequential scanning in thread errors
            return self._scan_sequential(directory, progress_callback)
    ...
    def _thread_scan_worker(self, dirs, thread_id, progress_callback):
        """Worker thread to scan a list of directories."""
        try:
            ...
            for directory in dirs:
                ...
                # Use an adapted deep search for this thread
                    ...
                    try:
                        ...
                        try:
                            with os.scandir(current_dir) as it:
                                ...
                                # Add subdirectaries to the stack
                                dirs_stack.extend(reversed(subdirs))  # Reverse order for DFS
                        except (PermissionError, OSError):
                            continue
    ...
    def _create_balanced_dir_chunks(self, dirs, num_chunks):
        ...
        for d in dirs:
            try:
                # Treasures directory size by counting the entries at the top level
                size = sum(1 for _ in os.scandir(d))
                dir_sizes.append((d, size))
            except (PermissionError, OSError):
                dir_sizes.append((d, 1))  # Minimum size for inaccessible directories

        # Sorting directories according to estimated size (descending)
        dir_sizes.sort(key=lambda x: x[1], reverse=True)

        # Distribute to Greedy-Algorithm (largest first)
        chunks = [[] for _ in range(num_chunks)]
        chunk_sizes = [0] * num_chunks

        for dir_path, size in dir_sizes:
            # Find chunk with the lowest current size
    ...
    def _adjust_adaptive_parameters(self, files_per_second, dirs_scanned, total_time):
        """Adjusts adaptive parameters for future scans based on performance metrics."""
        try:
            ...
            if dirs_scanned < 10 or total_time < 1.0:
                # Small directory structure - disabling threading
...
class OptimizedROMSorterGUI:
    ...
    def __init__(self):
        ...
        self._create_menu()  # Menu bar added
    ...
    def _initialize_window(self):
        ...
        # Create the main window based on the available drag & drop support
        if DND_AVAILABLE:
            try:
                ...
                if DND_MODE == "tkdnd":
                    ...
                    # Check whether tkinterdnd was correctly initialized
                ...
                print(f"Error creating window with Drag & Drop support: {e}")
                import traceback
                traceback.print_exc()

                # Fallback to standard Tk window
                self.root = tk.Tk()
                globals()['DND_AVAILABLE'] = False
                globals()['DND_MODE'] = "none"
    ...
    def _handle_root_drop(self, event):
        """Handle Drag & Drop events in the main window with robust path extraction."""
        try:
            ...
            # Use the improved path extraction
            paths = self._extract_drop_paths(event.data)

            if not paths:
                print("No valid paths im Root-Drop gefunden")
                return

            # Search first for valid directories
            for path in paths:
                if os.path.exists(path) and os.path.isdir(path):
                    print(f"Root-Drop: Valid directory found: {path}")
                    self._set_source_folder(path)
                    # Zeige visuelles Feedback
                    self._show_drag_drop_feedback(f"📁 {os.path.basename(path)} selected")
                    return

            # Then try parent lists of files
            for path in paths:
                if os.path.exists(path) and os.path.isfile(path):
                    parent_dir = os.path.dirname(path)
                    if os.path.exists(parent_dir) and os.path.isdir(parent_dir):
                        print(f"Root-Drop: Using parent directory: {parent_dir}")
                        self._set_source_folder(parent_dir)
                        # Zeige visuelles Feedback
                        self._show_drag_drop_feedback(f"📁 {os.path.basename(parent_dir)} selected")
                        return
    ...
    def _extract_drop_paths(self, data):
        """Extracts paths from drop data - reusable method for different drop targets."""
        try:
            # Use the existing implementation in optimized dropstore drain
            from inspect import getmembers, ismethod

            # Search for an instance of optimized drag drop in the subordinate widgets
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(label="Open source folder...", command=self._select_source_folder)
        file_menu.add_command(label="Open destination folder...", command=self._select_dest_folder)
    ...
    def _initialize_theme_support(self):
        ...
        if THEME_SUPPORT:
            try:
                ...
                print("Theme system successfully initialized")
            except Exception as e:
...
© 2024 ROM Sorter Team. Alle Rechte vorbehalten."""
    ...
    def _create_footer(self, parent):
        ...
        # Status label for database status and general status messages
    ...
    def _analyze_files(self, files: List[Path]) -> Dict[str, int]:
        ...
        if len(files) < 500:
            ...
            for file_path in files:
                ...
                # Special treatment for files with ambiguous extensions
                if ext in ('.bin', '.iso', '.img', '.cue', '.chd', '.rom'):
                    # In the event of ambiguous extensions, we try to get out of the file name or the overriding folder
                    # to get more information
                    filename_lower = file_path.name.lower()
                    parent_dir = file_path.parent.name.lower()

                    # Check console -specific keywords in file names or folder names
                    console_keywords = {
    ...
    def _cancel_sorting(self):
        ...
        def sort_worker():
            try:
                ...
                # Our own optimized file processing code regardless of modules_available
                try:
                    # Scanning and organizing the files directly in the GUI
                    source_dir = Path(self.source_folder.get())
                    dest_dir = Path(self.dest_folder.get())

                    # Collect all ROM files in the source directory
                    all_files = []
                    for root, dirs, files in os.walk(source_dir):
                        root_path = Path(root)

                        # For each file name in the current directory
                    ...
                    for file_path in all_files:
                        ...
                        # Improved console hypertension from file extension
                        ext = file_path.suffix.lower()
                        ...
                        try:
                            # Extended detection for ambiguous file extensions (.bin,.iso,.zip, etc.)
                            if console_type in ["Binary", "ROM_File", "Disk_Image", "Archive"]:
                                ...
                                try:
                                    ...
                                    # Use the archive detector for improved detection
                                    ...
                                    # If the archive detector cannot be imported
                                ...
                                try:
                                    ...
                                    # If the CHD detector cannot be imported
                                    ...
                                    if parent_dir.lower() not in ["roms", "games", "emulation"]:
                                        for console in self.CONSOLE_MAP.values():
                                            ...
                                            if console_name in parent_dir.lower():
                                                ...
                                                print(f"Detected console from directory name: {console}")
                                                break
                                ...
                                # 2. Try to use utils.detect_console_Fast if available and still generic
                                if console_type in ["Binary", "ROM_File", "Disk_Image", "Archive"]:
                                    try:
                                ...
                                if console_type == "Archive" and ext == '.zip':
                                    try:
                                        ...
                                        with zipfile.ZipFile(file_path, 'r') as zip_ref:
                                            ...
                                            # Filter directories and hidden files
                                        ...
                                        # If we have found consoles, we take the most common
                                        if console_counts:
                        ...
                        # Target file name determine with conflict treatment
                        dest_file = console_dir / file_path.name

                        # Treatment of name conflicts
                        counter = 1
                        ...
                        try:
                            # Actual moving the file with robust error handling
                            try:
                                shutil.move(str(file_path), str(dest_file))
                            except (shutil.Error, PermissionError) as move_error:
                                # Fallback: If moving fails to copy attempts and then delete
                            ...
                            # Improved error treatment with detailed logging
    ...
    def _sort_complete(self, result: Dict[str, Any], sort_state: Dict[str, Any] = None):
        ...
        if consoles:
            # Sorting consoles alphabetically for better readability
            consoles_sorted = sorted(consoles)
            # Show all consoles, not just the first couple
        ...
        # Show Completion Dialog with Report - Improved version with Modaler Display
    ...
    def _show_sort_results(self, title: str, report_lines: List[str]):
        ...
        # Create a new dialog and configure as a modal
        dialog = tk.Toplevel(self.root)
        dialog.title(title)
        dialog.transient(self.root)  # Bind dialog to main window
    ...
    def _collect_actual_statistics(self) -> Dict[str, int]:
        """Collects actual statistics from the destination folder."""
        try:
            dest_dir = Path(self.dest_folder.get())
            ...
            # Traverse all subdirectories in the destination folder
            for item in dest_dir.iterdir():
                if item.is_dir():
                    # Cute all files in the console directory
                    ...
                    for ext in self.CONSOLE_MAP.keys():
                        ...
                        # Tot files with this extension
                        file_count += len(list(item.glob(f"*{ext_norm}")))

                    # Add to statistics when files have been found
                    if file_count > 0:
    ...
    def _sort_error(self, error_msg: str, sort_state: Dict[str, Any] = None):
        ...
        # Use the improved method for displaying the error message
    ...
    def _close_all_dialogs(self):
        """Closes all active dialog windows."""
        try:
            # Close all the top level dialogues that could serve as success or error messages
            for widget in self.root.winfo_children():
                if isinstance(widget, tk.Toplevel) and widget.wm_title() in ["Erfolg", "Error", "Fehler"]:
                    try:
                        widget.destroy()
            ...
            print(f"Fehler beim Close von Dialogen: {e}")

    def _set_running_state(self, running: bool):
        """Set running state and update UI."""
        self.is_running = running

        # Close all existing dialogues when starting an operation
        if running:
...
# Add the method to the class subsequently
