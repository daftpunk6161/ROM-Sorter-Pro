# ROM-Sorter-Pro Documentation Index

Welcome to the ROM-Sorter-Pro documentation. This index provides navigation to all available documentation.

---

## Quick Start

- [README](../README.md) – Project overview and quick start
- [Troubleshooting](../TROUBLESHOOTING.md) – Common issues and solutions
- [User Manual](USER_MANUAL.md) – Complete user guide

---

## For Users

| Document | Description |
|----------|-------------|
| [User Manual](USER_MANUAL.md) | Complete guide for using ROM-Sorter-Pro |
| [Config Reference](CONFIG_REFERENCE.md) | Configuration options explained |
| [Feature Catalog](FEATURE_CATALOG.md) | Full feature list |
| [Screenshots](SCREENSHOTS.md) | UI screenshots |
| [Video Tutorial](VIDEO_TUTORIAL.md) | Video walkthroughs |

---

## For Developers

| Document | Description |
|----------|-------------|
| [Developer Guide](DEVELOPER_GUIDE.md) | Getting started with development |
| [Architecture](ARCHITECTURE.md) | System architecture overview |
| [API Reference](API_REFERENCE.md) | Public API documentation |
| [Test Strategy](TEST_STRATEGY.md) | Testing guidelines |
| [Identification Strategy](IDENTIFICATION_STRATEGY.md) | ROM identification logic |

---

## Technical Deep-Dives

| Document | Description |
|----------|-------------|
| [DAT Import & Index](DAT_IMPORT_AND_INDEX.md) | DAT file processing |
| [DB Entrypoints](DB_ENTRYPOINTS.md) | Database access patterns |
| [Igir Integration](IGIR_INTEGRATION.md) | Integration with Igir |
| [Integrations](INTEGRATIONS.md) | External tool integrations |

---

## Project Management

| Document | Description |
|----------|-------------|
| [Roadmap](ROADMAP.md) | Feature roadmap |
| [Release Process](RELEASE_PROCESS.md) | Release procedures |
| [Changelog](../CHANGELOG.md) | Version history |
| [Contributing](../CONTRIBUTING.md) | Contribution guidelines |

---

## Internal / Archived

| Document | Description |
|----------|-------------|
| [MVP Docs](MVP_DOCS.md) | MVP specification |
| [Issue Backlog](ISSUE_BACKLOG.md) | GitHub issue workflow documentation |
| [GUI Refactoring Proposal](GUI_REFACTORING_PROPOSAL.md) | UI architecture proposal |
| [archive/](archive/) | Archived audit/backlog documents |

### Archived Documents (Completed)

All major audit and backlog items have been completed and archived:

- `archive/RELEASE_BACKLOG_CHECKLIST.md` – Release checklist (100% P0-P2 complete)
- `archive/RELEASE_AUDIT_BACKLOG.md` – Audit findings (all fixed)
- `archive/AUDIT_DETECTION_SORTING.md` – Detection audit (all implemented)
- `archive/BACKLOG_REF3.md` – MVVM refactoring (complete)
- `archive/RELEASE_AUDIT_2026_01_28*.md` – Historical audits

---

*Last updated: 2026-01-30*
