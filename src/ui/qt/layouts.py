...
def _build_beginner_wizard(ctx: LayoutContext) -> object:
	...
	bar_layout = QtWidgets.QHBoxLayout(bar)
	bar_layout.setContentsMargins(10, 10, 10, 10)
	bar_layout.addWidget(QtWidgets.QLabel("Beginner Wizard"))
	bar_layout.addStretch(1)
	outer.addWidget(bar)
	...
	def _step_card(step: str, title: str, subtitle: str, icon_key: str) -> tuple[Any, Any]:
		...
		step_label.setObjectName("wizard_step")
	...
	card1, layout1 = _step_card("1", "Choose Source", "Quelle auswählen", "folder")
	btn1 = QtWidgets.QPushButton(label("Scan", "scan"))
	if ctx.on_action_scan is not None:
		btn1.clicked.connect(ctx.on_action_scan)
	layout1.addWidget(btn1)

	card2, layout2 = _step_card("2", "Choose Destination", "Ziel festlegen", "folder")
	btn2 = QtWidgets.QPushButton(label("Preview", "preview"))
	btn2.setObjectName("secondary")
	if ctx.on_action_preview is not None:
		btn2.clicked.connect(ctx.on_action_preview)
	layout2.addWidget(btn2)

	card3, layout3 = _step_card("3", "Execute", "Sortierung starten", "execute")
	btn3 = QtWidgets.QPushButton(label("Execute", "execute"))
	if ctx.on_action_execute is not None:
		btn3.clicked.connect(ctx.on_action_execute)
