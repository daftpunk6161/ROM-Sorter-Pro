#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Rome Sarter Pro - Enhanced Comment Translation Tool V2.1.8 This script Translates Both German Comments and Docstrings in Python Files to English, Preserving the Original Files As Backups. It uses GoogleTranslator or MymemoryTranslator as Fallback and Maintains a Translation Cache to Avoid Repeated Translations of the Same Text."""
import io
import re
import sys
import json
...
CACHE_FILE = ROOT / "comment-translation-cache.json"  # Cache in project root
...
def is_probably_german(s: str) -> bool:
    """
    Improved detection of German text in comments or docstrings.
...
def is_docstring(token) -> bool:
    """Check if a token is likely a docstring (a string at module, class or function level)"""
    return token.type == tokenize.STRING and token.string.startswith(('"""', "'''"))

def clean_docstring(s: str) -> str:
    """Clean Up a Docstring for Translation"""
    # Remove the quotes
...
def process_python_file(path: Path, cache: Dict[str, str]) -> bool:
    """Process a Python File to Translate German Comments and Docstrings to English. Args: Path: The Path to the Python File Cache: Translation Cache Dictionary Return: True If the File Was Changed, False OtherWise"""
    ...
    for tok in tokens:
        ...
        elif is_docstring(tok):
            # Handle docstrings
            docstring_text = clean_docstring(tok.string)
            if docstring_text and is_probably_german(docstring_text):
                new_text = translate_text_de_to_en(docstring_text, cache)
                # Preserve the original quotes and indentation
                quote_style = tok.string[:3]
                if tok.string.startswith('"""'):
                    new_tok_str = f'"""{new_text}"""'
                else:
                    new_tok_str = f"'''{new_text}'''"
                if new_tok_str != tok.string:
                    changed = True
...
def walk_and_translate(root: Path) -> None:
    ...
    """
    print(f"[INFO] Starting translation of German comments and docstrings in Python files...")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--ui-only":
        print("[INFO] Translating only UI files...")
        walk_and_translate(ROOT / "src" / "ui")
    else:
